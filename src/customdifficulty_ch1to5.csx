...
{
    importGroup.QueueFindReplace(scrName, "global.chapter = ", @$"
        ...
        global.diff_menupresetrow = undefined;
        global.diff_menucreateuserpresetrow = undefined;
        global.diff_menudeleteuserpresetrow = undefined;
        ...
        global.diff_saveuserpreset = function() {{
            ...
            ds_map_add(presetdata, ""hitall"", global.diff_hitall);
            ds_map_add(presetdata, ""iframes"", global.diff_iframes);
            ...
            ds_map_add(presetdata, ""downedregen"", global.diff_downedregen);
            ds_map_add(presetdata, ""victoryres"", global.diff_victoryres);
            ...
            ds_map_add(presetdata, ""enemyscaling"", global.diff_enemyscaling);
            ...
            for (var i = 0; i < array_length(newuserpresetnames); i++) {{
                if (i != 0)
                    newmetapresetsvalue += "";"";
                newmetapresetsvalue += newuserpresetnames[i];
            }}
            ini_write_string(""META"", ""PRESETS"", newmetapresetsvalue);
            ossafe_ini_close();

            var savedatarefs = global.diff_menu.save_data_refs();
            ossafe_ini_open(""difficulty.ini"");
            for (var i = 0; i < array_length(savedatarefs); i++) {{
                ini_write_real(""preset_"" + global.diff_saveuserpresetname, savedatarefs[i].ini_key, savedatarefs[i].get());
            }}
            ossafe_ini_close();

            if (!is_undefined(global.diff_menupresetrow)) {{
                var userpresetvaluerange = """";
                for (var i = 0; i < array_length(newuserpresetnames); i++) {{
                    userpresetvaluerange += "";"" + newuserpresetnames[i] + ""="" + newuserpresetnames[i] + ""`"";
                }}
                global.diff_menupresetrow.value_range = ""{string.Join(";", presets.Select(pair => @$"{pair.Key.ToUpper()}={pair.Key}`"))};CUSTOM=Custom`"" + userpresetvaluerange;
            }}

            if (!is_undefined(global.diff_menucreateuserpresetrow)) {{
                ...
                global.diff_menucreateuserpresetrow.value_range = createuserpresetsvaluerange;
            }}

            global.diff_deleteuserpresetname = """";
            if (!is_undefined(global.diff_menudeleteuserpresetrow)) {{
                ...
                global.diff_menudeleteuserpresetrow.value_range = deleteuserpresetsvaluerange;
            }}
        ...
        global.diff_deleteuserpreset = function() {{
            ...
            ossafe_ini_open(""difficulty.ini"");
            if (ini_section_exists(""preset_"" + global.diff_deleteuserpresetname))
                ini_section_delete(""preset_"" + global.diff_deleteuserpresetname);
            ossafe_ini_close();
        ...
        if (ossafe_file_exists(""difficulty.ini"")) {{
            ...
            for (var i = 0; i < array_length(userpresetnames); i++) {{
                ...
                ds_map_add(presetdata, ""enemyscaling"", ini_read_real(""preset_"" + userpresetnames[i], ""ENEMY_SCALING"", {presets[preset_default].enemyscaling.ToString().ToLower()}));
        ...
        {{
            switch (global.diff_preset) {{
                ...
                case ""Custom"":
                    ...
                    break;
                default:
                    if (ds_map_exists(global.diff_userpresets, global.diff_preset)) {{
                        var presetdata = ds_map_find_value(global.diff_userpresets, global.diff_preset);
                        global.diff_damagemulti = ds_map_find_value(presetdata, ""damagemulti"");
                        global.diff_gameboarddmgx = ds_map_find_value(presetdata, ""gameboarddmgx"");
                        global.diff_hitall = ds_map_find_value(presetdata, ""hitall"");
                        global.diff_iframes = ds_map_find_value(presetdata, ""iframes"");
                        ...
                        global.diff_downedregen = ds_map_find_value(presetdata, ""downedregen"");
                        global.diff_victoryres = ds_map_find_value(presetdata, ""victoryres"");
            ...
            global.diff_menu = global.modmenu.create({{
                ...
                apply: {{type: ""OnClose"", func: function () {{
                    ...
                    "))}
                        ...
                        var userpresetnames = ds_map_keys_to_array(global.diff_userpresets);
                        var foundPreset = false;
                        for (var i = 0; i < array_length(userpresetnames); i++) {{
                            var presetdata = ds_map_find_value(global.diff_userpresets, userpresetnames[i]);
                            if (global.diff_damagemulti == ds_map_find_value(presetdata, ""damagemulti"")
                                && global.diff_gameboarddmgx == ds_map_find_value(presetdata, ""gameboarddmgx"")
                                && global.diff_hitall == ds_map_find_value(presetdata, ""hitall"")
                                && global.diff_iframes == ds_map_find_value(presetdata, ""iframes"")
                                && global.diff_enemycd == ds_map_find_value(presetdata, ""enemycd"")
                ...
                form: [{{
                    ...
                    type: ""Toggle"",
                    title: ""Enemy Scaling"",
                    data_ref: {{var_name: ""diff_enemyscaling"", default_value: {presets[preset_default].enemyscaling.ToString().ToLower()}, ini_key: ""ENEMY_SCALING""}},
                    value_range: ""OFF=false;ON=true"",
                    change_func: global.diff_usepreset_custom
                }}, {{
                    type: ""Slider"",
...
if (ch_no == 5) {
    ...
    // Fix cooldowns applying to Pink's attack patterns
    ...
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "if (btimer >= (global.diff_enemycd * 6))", "if (btimer >= 6)");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 35);", "btimer -= 35;");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 25);", "btimer -= 25;");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 18);", "btimer -= 18;");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 11);", "btimer -= 11;");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 8);", "btimer -= 8;");
    // TODO cannot possibly fathom why this one doesn't work
    // importGroup.QueueRegexFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "else if \\(phase < 12\\)\\s+btimer -= ceil\\(global\\.diff_enemycd \\* 6\\)\\;", @"else if (phase < 12) btimer -= 6;");
    // importGroup.QueueRegexFindReplace("gml_Object_obj_dbulletcontroller_Step_0", @"btimer -= ceil\(global\.diff_enemycd \* 6\)\;\s*else", @"btimer -= 6; else");
    // SimpleTextOutput("piss","poop","else if \\(phase < 12\\)\\s+btimer -= ceil\\(global\\.diff_enemycd \\* 6\\)\\;", true);
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 99);", "btimer -= 99;");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * _timeadd);", "btimer -= _timeadd;");
    importGroup.QueueFindReplace("gml_Object_obj_dbulletcontroller_Step_0", "btimer -= ceil(global.diff_enemycd * 4);", "btimer -= 4;");
...
// Enemy classifications (used by Extra Enemies & Enemy Scaling)
Func<string, string> constructEnemyClassificationBlock(string[] ch1, string[] ch2, string[] ch3, string[] ch4, string[] ch5, string[] ch6, string[] ch7) {
    string[] thisch = {};
    switch (ch_no) {
        case 0:
            List<string> demo = new List<string>();
            foreach (string entry in ch1) {
                demo.Add($"{entry}_ch1");
            }
            demo.AddRange(ch2);
            thisch = demo.ToArray();
    ...
    return (string index) => $"(global.monsterinstancetype[{index}] == {string.Join($" || global.monsterinstancetype[{index}] == ", thisch)})";
}
Func<string, string> checkregularenemyblock = constructEnemyClassificationBlock(
    new string[] {"obj_ponman_enemy", "obj_rudinnranger", "obj_diamondenemy", "obj_heartenemy", "obj_rabbick_enemy", "obj_bloxer_enemy", "obj_jigsawryenemy", "obj_headhathy"},
    new string[] {"obj_ponman_enemy", "obj_rudinnranger", "obj_omawaroid_enemy", "obj_poppup_enemy", "obj_tasque_enemy", "obj_werewire_enemy", "obj_maus_enemy", "obj_virovirokun_enemy", "obj_swatchling_enemy", "obj_werewerewire_enemy"},
    new string[] {"obj_ponman_enemy", "obj_rudinnranger", "obj_rabbick_enemy", "obj_shadowman_enemy", "obj_zapper_enemy", "obj_pippins_enemy", "obj_ribbick_enemy"},
    new string[] {"obj_ponman_enemy", "obj_rudinnranger", "obj_swatchling_enemy", "obj_guei_enemy", "obj_balthizard_enemy", "obj_bibliox_enemy", "obj_mizzle_enemy", "obj_bell_enemy", "obj_halo_enemy", "obj_organ_enemy",
        "obj_titan_spawn_enemy", "obj_pippins_enemy", "obj_zapper_enemy", "obj_ribbick_enemy"},
    new string[] {"obj_ponman_enemy", "obj_rudinnranger", "obj_floradinn_enemy", "obj_leafling_enemy", "obj_scarecrow_enemy", "obj_kawkaw_enemy", "obj_shinobeetle_enemy", "obj_sheary_enemy"},
...
Func<string, string> checkbossenemyblock = constructEnemyClassificationBlock(
    new string[] {"obj_joker", "obj_king_boss"},
    new string[] {"obj_queen_enemy", "obj_spamton_neo_enemy", "obj_gigaqueen_enemy"},
    new string[] {"obj_knight_enemy", "obj_tenna_enemy"},
    new string[] {/* Game doesn't allow you to win through violence & has no fallback "obj_hammer_of_justice_enemy", "obj_titan_enemy" */},
    new string[] {/* Game doesn't allow you to win through violence & has no fallback "obj_pink_enemy", "obj_flowery_enemy" */},
    new string[] {/* REAL (no source required): "obj_nightmare_enemy", "obj_knight_enemy" */},
    new string[] {/* REAL (no source required): "obj_copycat_enemy", "obj_knight_enemy_b", "obj_truetitans_enemy_c", "obj_ralseiangel_enemy", "obj_noelleangel_enemy_b", "obj_mysterymanangel_enemy_c"*/}
);
    ...
    foreach ((string ScrName, string Replace) script in scriptstodo)
    {
        importGroup.QueueFindReplace(script.ScrName, script.Replace, @$"
            ...
            {{
                ...
                for (i = 0; i < 3; i += 1)
                {{
                    if (global.monstertype[i] > 0 && {checkregularenemyblock("i")})
...
// Apply Enemy Scaling
importGroup = startCodeGroup("Enemy Scaling", "Enemy Scaling", false);
{
    (string ScrName, string Replace, string Index) [] scriptstodo = {};
    if (ch_no == 1) {
        (string ScrName, string Replace, string Index)[] ch1scriptstodo = {("gml_Object_obj_battlecontroller_Create_0", "event_user(12);", "i"), ("gml_Object_obj_lancerboss3_Step_0", "event_user(12);", "0")};
        scriptstodo = scriptstodo.Concat(ch1scriptstodo).ToArray();
    }
    if (ch_no == 0) {
        (string ScrName, string Replace, string Index)[] demoscriptstodo = {("gml_Object_obj_battlecontroller_ch1_Create_0", "event_user(12);", "i"), ("gml_Object_obj_lancerboss3_ch1_Step_0", "event_user(12);", "0")};
        scriptstodo = scriptstodo.Concat(demoscriptstodo).ToArray();
    }
    if (ch_no == 2) {
        (string ScrName, string Replace, string Index)[] ch2scriptstodo = {("gml_Object_obj_ch2_scene9_Step_0", "event_user(12);", "myself")};
        scriptstodo = scriptstodo.Concat(ch2scriptstodo).ToArray();
    }
    if (ch_no >= 2 || ch_no == 0) {
        (string ScrName, string Replace, string Index)[] ch2scriptstodo = {("gml_GlobalScript_scr_monster_makeinstance", "event_user(12);", "arg0")};
        scriptstodo = scriptstodo.Concat(ch2scriptstodo).ToArray();
    }

    foreach ((string ScrName, string Replace, string Index) script in scriptstodo)
    {
        importGroup.QueueFindReplace(script.ScrName, script.Replace, @$"
            {script.Replace}

            if (global.diff_enemyscaling) {{
                if ({checkregularenemyblock(script.Index)}) {{
                    global.monstermaxhp[{script.Index}] = ceil(global.diff_healthregular * global.monstermaxhp[{script.Index}]);
                    global.monsterhp[{script.Index}] = ceil(global.diff_healthregular * global.monsterhp[{script.Index}]);
                }}
                if ({checkminibossenemyblock(script.Index)}) {{
                    global.monstermaxhp[{script.Index}] = ceil(global.diff_healthminiboss * global.monstermaxhp[{script.Index}]);
                    global.monsterhp[{script.Index}] = ceil(global.diff_healthminiboss * global.monsterhp[{script.Index}]);
                }}
                if ({checkbossenemyblock(script.Index)}) {{
                    global.monstermaxhp[{script.Index}] = ceil(global.diff_healthboss * global.monstermaxhp[{script.Index}]);
                    global.monsterhp[{script.Index}] = ceil(global.diff_healthboss * global.monsterhp[{script.Index}]);
                }}
            }}
        ");
    }
